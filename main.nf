...
process sniffles_paired {
    ...
    input:
        ...
        file tumour_snf
    output:
        path "paired_sniffles.vcf", emit: vcf
    script:
    """
    sniffles --input ${normal_snf} ${tumour_snf} --vcf paired_sniffles.vcf
    """
}
...
workflow {
    ...
    tumor_bam = file(params.tumor_bam, type: "file")
    if (!tumor_bam.exists()) {
        println("--tumor_bam: File doesn't exist, check path.")
        exit 1
    }
    tumor_bai = file(params.tumor_bai, type: "file")
    if (!tumor_bai.exists()) {
        println("--tumor_bai: File doesn't exist, check path.")
        exit 1
    }

    normal_bam = file(params.normal_bam, type: "file")
    if (!normal_bam.exists()) {
        println("--normal_bam: File doesn't exist, check path.")
        exit 1
    }
    normal_bai = file(params.normal_bai, type: "file")
    if (!normal_bai.exists()) {
        println("--normal_bai: File doesn't exist, check path.")
        exit 1
    }
    ...
    sniffles_paired(tumor_results.out.snf, normal_results.out.snf)
